...
- **Newest completed device diagnostic:** [WCN3990 HTT key diagnostic](WIFI-KEYINSTALL-HTT-2026-08-15.md) — the one controlled hostapd association succeeded and received a matching pairwise `SEC_IND`; the captured `-110` occurred instead on pairwise `DEL_KEY` during controlled client teardown, with no matching indication observed through the long tail. This is lost from the host's perspective; the firmware delete-ack contract remains open. SMMU stream-0x1900 fault count stayed fixed at five, so those faults are not the proximate cause of this timeout. Recovery to LineageOS passed. Do not replay this image/association merely to repeat the classification.
...
- **Newest host-only checkpoint:** [WCN3990-DELETE-KEY-HOST](WCN3990-DELETE-KEY-HOST-2026-08-15.md) — three signed joan/ath10k pairwise-delete-key-wait commits and a clean `Image.gz`/`dtb`/`modules` build sealed. No boot.img was packaged. Lance held off the RAM-only association/tether test; every device gate remains open and needs a fresh named approval. Previous host-only checkpoint: [GPU-IREF-HOST](GPU-IREF-HOST-2026-08-11.md).
...
| [WIFI-KEYINSTALL-HTT](WIFI-KEYINSTALL-HTT-2026-08-15.md) | 2026-08-15 | CLASSIFIED: association PTK `SEC_IND` matched peer 30 and arrived on time; no matching indication was observed for the later pairwise `DEL_KEY`, which timed out `-110`. Lost from the host's perspective; firmware generation remains open. SMMU fault count did not change. | One-shot consumed; LineageOS recovery passed. Do not replay merely to repeat the split. Next host-only stack: [WCN3990-DELETE-KEY-HOST](WCN3990-DELETE-KEY-HOST-2026-08-15.md). |
| [WCN3990-DELETE-KEY-HOST](WCN3990-DELETE-KEY-HOST-2026-08-15.md) | 2026-08-15 | HOST-ONLY PASS: pairwise delete-key wait quirk committed and host-built (`7.2.0-rc2-g834154d6b082`). boot.img not packaged; RAM test held by owner. | Do not boot from this packet. Fresh approval must name a future image SHA. Handoff: `../aurel-handoff-2026-08-15-to-next-session-wcn3990-delete-key-ram-test.md`. |
