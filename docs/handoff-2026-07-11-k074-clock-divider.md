...
Date: 2026-07-11
For: Lance + Hermes Agent / next session

## One-paragraph state

The display pipeline is now END-TO-END up: mmss SMMU probes (Hermes Agent-assisted), DSI
...
carry a valid signal. This is the same /4-vs-/2 divider issue Hermes Agent chased
...
rate. Hermes Agent's K070 logged the saved outdiv as /2 (0x1) but the live tree
...
- K068 (Hermes Agent-assisted): CLK_OPS_PARENT_ENABLE on byte0/pclk0 → RCG warnings gone but
  ...
  tree, rejected.
- K072 (Claude Code-assisted): init-only nonzero vco_current_rate seed (recalc kept pure) →
...
Conventions: no force-push; Assisted-by = live model (Hermes Agent Hermes:<current>);
...
`out/boot-joan-20260711-k075-panel-instr.img` sha256
...
patch `out/20260711-k075-panel-instrumentation.patch`).
