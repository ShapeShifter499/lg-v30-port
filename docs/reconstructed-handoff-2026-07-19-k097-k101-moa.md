# Reconstructed technical handoff — M4 complete; GPU K098-K101 paused; K092 provenance resolved

Original-work-under-review: Claude Code and Lance, as attributed in the source records
...
> **Reconstruction notice:** This is not an Claude Code-authored final handoff. Hermes Agent
> reconstructed it after Claude Code stopped at the usage limit. Unless explicitly
> quoted or attributed to a source, the correlation, classifications, caveats,
> and corrections in this document are Hermes Agent's evidence-backed synthesis.

## Why this handoff exists

Claude Code's last formal handoff ends at K095:

- `docs/handoff-2026-07-19-night2-dsi-ctrl-session1.md`

Claude Code then continued through K096-K101, updated the kernel ledger, and hit the
Claude usage limit immediately after Lance asked Claude Code to ask Hermes Agent about an
...
### Why a later Claude Code continuation did not recognize Claude Code's own K092 code

This was not simply one continuous Claude Code session forgetting an edit. The logs
...
1. At `22:15:26`, background Claude Code reset `dsi_host.c` to the git version.
2. At `22:15:48`, foreground Claude Code's K092 script tried to replace text inside
...
3. At `22:16:01`, background Claude Code independently wrote `cfg_pending`,
   `clk_rcg2_replay_enable()`, and the byte/pixel `.enable` hooks, then started
   its own K092 build.
4. At `22:22:53`, foreground Claude Code staged `dsi_host.c` and MMCC for
...
- `out/reconstructed-20260720-k092-k101/k092-split-session-causality-20260720.txt`
...
3. **The exact “mystery” clock implementation is resolved. It is Claude Code's K092
   experiment, not Hermes Agent's and not a verbatim upstream import.** Related
   ...
   `clk_rcg2_replay_enable()` function, or byte/pixel-op hook. Claude Code authored
...
4. **Do not boot the current generic gadget image.**
   `out/boot-joan-mainline.img` was packaged for K101 before Claude Code reverted the
...
GDSCs forced `ALWAYS_ON`. Claude Code tested that image, observed the same hard wedge,
...
fastboot device was active. Hermes Agent sent no commands to the phone and performed no
...
- `out/reconstructed-20260720-k092-k101/current-kernel-worktree-full.patch`
...
|---|---|---|
| `clk-rcg.h`, `clk-rcg2.c` | Claude Code K092 | Generic pending-config replay experiment; built/tested, not committed; accidentally left dirty |
| `panel-lg-sw43402.c` | Claude Code K093 | DCS/BTA readback probes; intentional debug instrumentation; saved patch exists |
| `msm8998-lge-joan.dts` | Claude Code K099/K100 | GPU reserved-memory relocation, GPU enable/zap node, PM8005 floor and `vdd-supply`; enables the wedging path; do not commit as-is |
...
The final Claude Code session incorrectly concluded that the clock code might be
Hermes Agent's. The logs resolve it conclusively:
...
- Claude Code's own description immediately afterward:
  “K092 config-replay-at-enable” / “the cleanest design yet”
- saved full experiment patch:
  `out/20260719-k092-full-worktree.patch`
- patch sha256:
...
It was not an Hermes Agent edit and was not unsaved.
...
- Claude Code's authoring log contains no upstream/shared-op lookup. Immediately
  before writing K092, Claude Code inspected `clk_byte2_ops` and `clk_pixel_ops`,
...
byte/pixel ops. The evidence therefore supports **Claude Code as author of the exact
...
Claude Code then saved the full K092 diff and split out/committed the brightness
...
- The background-continuation log shows why: at JSONL line 1279 Claude Code ran
  `git checkout -- drivers/gpu/drm/msm/dsi/dsi_host.c`. Twenty-two seconds
  later, foreground Claude Code's K092 `str.replace()` looked for the now-absent
...
- snapshot: `out/config-20260720-k098-gpucc`
...
- `out/reconstructed-20260720-k092-k101/FIRMWARE_SHA256SUMS`
...
- `out/20260720-k099-k100-gpu-enable-UNCOMMITTED.patch`
...
- zap loading proceeded without an SCM authentication error; Claude Code recorded
...
Claude Code's ledger says “interconnect ruled out.” The evidence is narrower:
...
2. VDDCX/CPR/GX voltage sequencing is incomplete under load; mainline lacks the
   MSM8998 CPR path Claude Code expected.
...
- `out/reconstructed-20260720-k092-k101/`
...
- `k092-split-session-causality-20260720.txt`
...
Do not rewrite Claude Code's historical entries. Append corrections with current
attribution.
...
4. Inspect the archived pre-cleanup dirty diff and the two saved Claude Code patches;
...
Hermes Agent has not started a successor device experiment. The phone is recovered,
...
its hashes and build record remain. The still-open Claude Code CLI was suspended with
