...
## Hermes Agent follow-up — panel-only bisect complete; capped variant pending
...
controls for the device and pmOS rootfs. Lance also reports Claude Code confirmed
...
## Hermes Agent audit correction — k172/k175 was not a controlled kernel-only pair
...
- Claude Code's floor6 and max255 manual-brightness images also include that gate;
...
- command line: byte-preserved from k172 and contains
  `msm.k127_no_suspend=1`;
- one-shot runner: `/tmp/k178-ramboot-once.sh` on <usb-host>, SHA-256
...
## Hermes Agent K178 device result — GUI slider PASS
...
`dmesg` on <usb-host>, after a clean `RAMBOOT_OK` (send 0.592 s, boot 5.094 s):
...
`/tmp/ramboot-joan.sh` on <usb-host> does this end to end from either LineageOS
or a wedged state:

    ssh <usb-host-ssh-alias> '/tmp/ramboot-joan.sh /tmp/boot-joan-k174.img'
...
(this orientation gets reversed every single time); key `<device-ssh-key>` on
<usb-host>, user `user`. From pmOS, busybox `reboot` cannot pass a mode string,
