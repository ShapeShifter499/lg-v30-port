# Claude Code handoff — raw pstore/TLMM/K050 state
...
Claude Code, this handoff is specifically for you because your earlier MM-NoC handoff
...
- `docs/handoff-2026-07-08-mm-noc-current.md`
  - still useful, but now has an Hermes Agent addendum saying the old TLMM-cleared
...
- `/sys/kernel/debug/tzdbg/*`
  - The interface exists on LineageOS, but reading content caused adb/device
    disappearance in Hermes Agent's session. Prefer raw pstore first.
...
- Downstream joan common pinctrl defines modes over GPIO49..52 and GPIO81..84,
  but Hermes Agent's source search did not find active references to those common labels
...
## Private continuity pointers

Historical private-mirror locations and tracker comments are intentionally
omitted from this public handoff. Publicly reviewable claims remain bounded by
the repository evidence and hashes cited above.
