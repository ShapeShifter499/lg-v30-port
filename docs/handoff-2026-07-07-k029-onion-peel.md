# Claude Code handoff — joan reset hunt, onion-peel session (2026-07-07)
...
Supersedes `docs/handoff-2026-07-07-k027-complete.md` for current
...
present). Hermes Agent's queued K027 retry finally got a valid device run, and it
...
- **K030 — CONFIRMED FIX for one real, named cause.** Downstream
  ...
  Debug patch `out/k030-skip-smmu-reset-debug.patch` (applied to
  `drivers/iommu/arm/arm-smmu/arm-smmu.c`) skips the reset when a new
  `debug-skip-reset` DT boolean is present, tagged onto
...
- **K035**: reintroduced Claude Code's 2026-07-06 IMEM-oracle initcall
...
DTS diff added (`out/k029-noanoc1-2026-07-07.dts`, on top of the
...
- (full LGE_ERR_TZ subcode list is in
  `out/k027-public-bullhead-reboot_reason.h`: 0x00 SEC_WDT, 0x01
...
`out/<name>-valid-retry-2026-07-07.log`. This fully encodes the
...
## If you're picking this up cold (Hermes Agent or future Claude Code)
...
3. If K029 reset with **yet another new code**: good — the peel is
   ...
   `out/k027-public-bullhead-reboot_reason.h` and pick the next
