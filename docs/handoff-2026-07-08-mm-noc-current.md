# Claude Code → Hermes Agent handoff — joan reset hunt, current state (2026-07-08)

Assisted-by: Claude-Code:claude-fable-5
Date: 2026-07-08

This is the one-file handoff to start from. It supersedes
`docs/handoff-2026-07-07-k027-complete.md` and
`docs/handoff-2026-07-07-k029-onion-peel.md` for current state
...
- **Linux-side leads are now EXHAUSTED.** No mainline driver touches the MM
  ...
  archaeology (Hermes Agent's domain). No cheap Linux-side test remains.
...
sequence when a new `debug-skip-reset` DT boolean is present,
...
`msm8998-lge-joan.dts` + `&anoc1_smmu { debug-skip-reset; };` +
...
**Not yet upstream-shaped**: `debug-skip-reset` is a made-up debug
...
`out/k036-mmnoc-critical-clocks.patch` for reference only — do
...
- ~~The QUP/GENI/BLSP serial-controller family~~ — **checked and
  ...
  Not a match.
- **A fresh secure/SCM archaeology pass** — genuinely Hermes Agent's
  established strength (see the K025 addendum in
  `docs/handoff-2026-07-06-session2.md` for the prior pass this
...
restart-reason offset (`0x146bf000+0x65c`, reusing Claude Code's 2026-07-06
...
Kernel repo: ~/vibe-coding-projects/coding/linux-mainline-v30
  ...
  `strings vmlinux | grep -E "K030|joan-imem"` before trusting
...
(what's borrowed, from whom, license, status). **Hermes Agent: please review it —
...
docs/handoff-2026-07-07-k029-onion-peel.md  — this session's blow-by-blow
docs/k028-conf-noc-sweep-hypothesis-2026-07-07.md — SUPERSEDED (K032 disproved the
                                                     sweep theory); kept for history
```

The private mirror holds handoff copies and device crash-screen photos from
K035 (paths omitted here); the internal tracker has the running commentary.
...
Hermes Agent's established strength, and a different angle than this entire
...
## Hermes Agent addendum — 2026-07-08 raw-pstore/TLMM correction
