# Handoff — M3 done, M4 gated on the mmss SMMU (for Lance + Hermes Agent)
...
- ssh: `ssh user@172.16.42.1`, password `[REDACTED]`, key `<device-ssh-key>`.
  sudo needs the password + a tty (`ssh -tt ... 'echo [REDACTED] | sudo -S ...'`).
...
## Conventions (binding — same for Hermes Agent)

- Commit trailers: `Signed-off-by: Lance <Gero3977@gmail.com>` +
  `Assisted-by: <harness>:<model actually running at write time>` — always
  the live model, never a copied/stale string. Harness model IDs change over
  time; write the provider-qualified route actually running. NEVER
  `Co-Authored-By`. Detailed
...
- Public docs use `Assisted-by: <harness>:<provider>/<model>` plus date/scope;
  private persona names and peer IDs stay in local-only continuity.
