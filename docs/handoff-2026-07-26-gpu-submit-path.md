...
device at /tmp/msmprobe and on <usb-host> at /tmp/msmprobe).
...
- BUILD on <build-host>: worktree `~/vibe-coding-projects/coding/<historical-k104-worktree>`
  (branch `joan/gpu-bringup`, pushed @ 9dc8eb3bc), build dir `../build-k104`,
  26 s incremental. FLASH/BOOT only from <usb-host> (xHCI cannot talk to LG aboot).
- Boot: `/tmp/boot-dev.sh <img> <sha256>` on <usb-host> (RAM-only `fastboot boot`;
...
- Device access: `ssh user@172.16.42.1` (key `<device-ssh-key>`, <usb-host>);
  root = `echo [REDACTED] | sudo -S …` (needs `ssh -tt`). Clean return to
  LineageOS: `sudo reboot`. Password documented in
  `docs/handoff-2026-07-11-m4-smmu-next.md`.
- Host gadget link comes up DOWN: `ip link set enp0s29u1u5 up` +
  `ip addr add 172.16.42.2/24` on <usb-host>.
- STREAM ALL LOGS OFF-DEVICE PER-LINE (`/tmp/nest-cmd.sh`, `/tmp/nest-probe.sh`
  on <usb-host>); the SoC resets hard enough to destroy on-device logs. Positive-
