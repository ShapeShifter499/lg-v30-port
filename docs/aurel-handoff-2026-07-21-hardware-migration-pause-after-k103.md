# Aurel handoff — hardware-migration pause after K103
...
- sealed K103 evidence directory:
  `out/k103-aurel-touch-no-input-enable-20260721/`;
...
- `docs/aurel-reconstructed-handoff-2026-07-19-k097-k101-moa.md`;
...
`~/vibe-coding-projects/coding/linux-mainline-v30-aurel-test/` is clean at
...
read back after writing. Honcho workspace `nymmesh-local` also contains the
shared pause conclusion for peers `human-ransu` and `agent-aurel`; several
...
`/mnt/aurel-backup-ssd/migration-checkpoints/lg-v30-k103-pause-20260721T180143Z/`
...
- `~/.ssh/id_pi_migration*`;
- `~/.ssh/github_gero3977_ed25519_signing*`;
- `~/.hermes/secrets/` and `~/.hermes/.env`;
- GitHub authentication stores;
- Nextcloud credentials/TOTP material;
- Honcho/Postgres, OpenClaw/Nymble, and Hermes runtime databases/configuration
