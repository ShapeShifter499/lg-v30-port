...
Every DSI rate is now correct. The panel is still black, which — per Hermes Agent's
...
Root cause (from Hermes Agent K076/K077): mainline msm8998 parents
...
Artifact: `out/20260711-k078-byte-intf-divider.patch`. It fixes a real
...
The pipeline is up with correct clocks; the panel just never shows. Hermes Agent's
...
the only real fix.
Artifact: `out/20260711-k079-simplefb-inherit.patch`.
...
- Harness `lg-v30-port`: ledger K078/K079 committed locally; nothing pushed
  ...
  model (Hermes Agent now Hermes:gpt-5.6-sol).
