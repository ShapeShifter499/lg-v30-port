...
#
# Usage: KDIR=<kernel-dir> A530_FW_DIR=<dir> ./make-pmos-image-fw.sh [reference.img] [output.img]
# Env:   A540_FW_DIR (default firmware/zap), WCN_FW_DIR (default firmware/wcn),
#        EXTRA_CMDLINE (appended), RAMDISK_OFFSET (default 0x02000000).
# Firmware sourcing: docs/joan-firmware-and-package-requirements.md
set -euo pipefail

. "$(dirname "$(readlink -f "$0")")/scripts/lib/bootimg.sh"
need_tools mkbootimg unpack_bootimg cpio gzip

REF="${1:-$JOAN_OUT/boot-joan-pmos-touch.img}"
DEST="${2:-$JOAN_OUT/boot-joan-pmos-fw.img}"
A530_FW_DIR="${A530_FW_DIR:-}"
A540_FW_DIR="${A540_FW_DIR:-$JOAN_REPO/firmware/zap}"
WCN_FW_DIR="${WCN_FW_DIR:-$JOAN_REPO/firmware/wcn}"

[[ -n "$A530_FW_DIR" ]] ||
	die "A530_FW_DIR is required; point it at lib/firmware/qcom extracted from the official postmarketOS firmware-qcom-adreno-a530 package"
...
for f in "${required_fw[@]}"; do
	[[ -s "$f" ]] || die "required GPU firmware missing or empty: $f"
done
...
unpack_cpio "$WORK/ramdisk" "$WORK/rd"
...
joan_mkbootimg "$WORK/Image.gz-dtb" "$WORK/ramdisk.new" "$CMDLINE" "$DEST"
